...
const Testimonials: React.FC<TestimonialsProps> = ({
  ...
  return <section className={cn("py-20", className)}>
      <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeading title="Client Testimonials" subtitle="Hear what our clients have to say about their experience working with Tensor Garden." centered />
        
        <div className="grid md:grid-cols-3 gap-8 mt-12">
          {testimonials.map((testimonial, index) => <Card key={index} className="card-shadow border-0 shadow-lg hover:shadow-xl transition-all animate-fade-in bg-white" style={{
